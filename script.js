...
// (((\d{1,3}\.){3}\d{1,3})( - ((\d{1,3}\.){3}\d{1,3})|/\d{1,3})?)
    ...
    function findMostLikely(server) {
        const name = server.name.toLowerCase();
        const ip = server.query.split(":")[0];
        const port = server.query.split(":")[1];
    ...
    function init() {
        ...
        const LATEST_SERVER_VERSION = 2320779165;
        const gs = {
            serverVersion: {
                24371034: "v14.?",
                572092818: "v15.2",
        ...
        function tryUpdateInfoModal() {
            ...
            const host = findMostLikely(info)
        ...
        socket.on("list-update", function (message) {
            ...
            try {
                ...
                function statsCount(server) {
                    ...
                    const host = findMostLikely(server)
